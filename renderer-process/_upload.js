...
let _queue_ = {
  ...
  remove_many: function (transfer_id) {
    ...
    this.items = this.items.filter(
      (single) => single.indexOf(`${transfer_id}::`) !== 0
    );
  },
...
function do_transfer(
  source_series_id = 'initial',
  source_upload_success = true
) {
  ...
  db_uploads.listAll((err, my_transfers) => {
    let _list_all_took = ((performance.now() - _list_all_timer) / 1000).toFixed(
      2
    );
    ...
    my_transfers.forEach((transfer) => {
      ...
      if (
        transfer.xnat_server === xnat_server &&
...
async function doUpload(transfer, series_id) {
  ...
  let selected_series = transfer.series.find(
    (ss) => series_id == ss[0].seriesInstanceUid
  );
...
function get_temp_upload_path() {
  return user_settings.get('temp_folder_alternative')
    ? user_settings.get('temp_folder_alternative')
    : path.join(tempDir, '_xdc_temp');
}

let upload_counter = 0;
async function copy_and_anonymize(
  transfer,
  series_id,
  filePaths,
  contexts,
  ...
  let xnat_server = transfer.xnat_server,
    user_auth = auth.get_user_auth(),
    table_row = transfer.table_rows.find(
      (tbl_row) => tbl_row.series_id == series_id
    );
  ...
  let request_settings = {
    ...
    url:
      xnat_server +
      `/data/services/import?import-handler=DICOM-zip&PROJECT_ID=${project_id}&SUBJECT_ID=${subject_id}&EXPT_LABEL=${expt_label}${qs}&overwrite=${overwrite}&rename=true&prevent_anon=true&prevent_auto_commit=true&SOURCE=uploader&autoArchive=AutoArchive` +
      '&XNAT_CSRF=' +
      csrfToken,
  ...
  async function store_checksums(transfer_id, series_id, upload_id) {
    ...
    let selected_series = current_transfer.series.find(
      (ss) => series_id == ss[0].seriesInstanceUid
    );
    ...
    st_item.forEach((sfile) => {
      let selected_item = selected_series.find(
        (item) => item.filepath == sfile.source
      );
  ...
  axios(request_settings)
    ...
    .then(async (data) => {
      ...
      console_red('mark_uploaded.then()', {
        series_id,
        transfer_series_ids,
        items_in_queue,
      });
      ...
      if (transfer.series_ids.length === 0) {
        ...
        let commit_url =
          xnat_server +
          $.trim(res.data) +
          '?action=commit&SOURCE=uploader&XNAT_CSRF=' +
          csrfToken;
        ...
        if (transfer.anon_variables.hasOwnProperty('tracer')) {
          let label =
            transfer.session_data.modality.indexOf('MR') >= 0
              ? 'xnat:petMrSessionData/tracer/name'
        ...
        axios
          .post(commit_url, commit_data, commit_request_settings)
          .then((commit_res) => {
            ...
            if (commit_res.data.indexOf(reference_str) >= 0) {
              ...
              let str_start =
                commit_res.data.indexOf(reference_str) + reference_str.length;
              ...
              return db_uploads._updateProperty(
                transfer.id,
                'session_link',
                session_link
              );
          ...
          .then((num_updated) => {
            ...
            if (num_updated) {
              Helper.notify(
                `Upload is finished. Session: ${transfer.url_data.expt_label}`
              ); // session label
              nedb_logger.success(
                transfer.id,
                'upload',
                `Session ${transfer.url_data.expt_label} uploaded successfully.`,
                transfer.url_data
              );
...
function handleUploadError(transfer, series_id, err) {
  ...
  if (err instanceof MizerError) {
    ...
    let err_msg_search =
      'File posts must include the file directly as the body of the message';
    if (
      err.response &&
      err.response.status === 400 &&
      err.response.data.indexOf(err_msg_search) > 0
    ) {
      log_and_respawn = false;
...
async function copy_and_anonymize_zip(
  transfer,
  series_id,
  _files,
  contexts,
          ...
          axios
            ...
            .catch((err) => {
              ...
              if (err.response.status != 301) {
                ...
                db_uploads
                  ._updateProperty(transfer.id, 'session_link', session_link)
                  .then((num_updated) => {
...
async function update_transfer_summary(
  transfer_id,
  property,
  new_value,
  callback = false
...
function get_transfer_from_transfer_progress(transfer, transfer_progress) {
  let my_transfer = transfer_progress.find(
    (tr) => tr.transfer_id == transfer.id
  );
...
async function update_progress_details(
  transfer,
  table_row,
  filesize,
  reset = false
  ...
  let my_transfer = get_transfer_from_transfer_progress(
    transfer,
    transfer_progress
  );
  ...
  if (selected_row.count == table_row.count && selected_row.db == 0) {
    ...
    let tbl_row = transfer_copy.table_rows.find(
      (t_row) => t_row.id === table_row.id
    );
...
window.onerror = function (errorMsg, url, lineNumber) {
  electron_log.error(
    `[Custom Uncaught Error]:: ${__filename}:: (${url}:${lineNumber}) ${errorMsg}`
  );
