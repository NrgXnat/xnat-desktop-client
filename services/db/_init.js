...
module.exports = (db_id, user_specific = true) => {
    ...
    if (db_index >= 0) {
        if (db_cache[db_index].sha1 === db_sha1) { // return from cache
            ...
            db_cache[db_index].instance.persistence.stopAutocompaction();
    ...
    db_instance.persistence.setAutocompactionInterval(60000);
    db_instance.on('compaction.done', function() {
        electron_log.info(`Database compaction is done (${db_filename})`);
    })
